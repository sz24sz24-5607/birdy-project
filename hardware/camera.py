...
class CameraController:
    ...
    def record_video_dynamic(self, output_path, pir_sensor, max_duration=None, absence_threshold=None):
        """
        Nehme Video auf solange der Vogel da ist (PIR-basiert).

        Startet rpicam-vid und stoppt die Aufnahme wenn:
        - PIR für absence_threshold Sekunden LOW ist (Vogel weg), oder
        - max_duration Sekunden erreicht wurden
        ...
        try:
            ...
            del self.camera
            time.sleep(1)
            logger.debug("Camera closed for rpicam-vid (dynamic)")
            ...
            # Überwache PIR-State und stoppe wenn Vogel weg
            ...
            while proc.poll() is None:
                ...
                if pir_sensor is not None and elapsed >= min_recording_duration:
                    ...
                    if pir_active:
                        pir_low_since = None  # PIR wieder HIGH → Vogel da
                    else:
    ...
    def extract_candidate_frames(self, video_path, n_frames=None, actual_duration=None):
        """
        Extrahiere gleichmässig verteilte Frames für Best-Frame-Selektion.

        Bei dynamischen Aufnahmen proportional mehr Frames (2 fps), mindestens 8.
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.info(
                    f"Extracted {len(frames)} candidate frames from {video_path.name} "
